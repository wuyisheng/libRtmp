...
class MyRenderer implements IRenderer {
    ...
    private CameraOpenDelegate mCameraOpenListener;
    ...
    MyRenderer(GLSurfaceView view) {
        mView = view;
        mEffect = Effect.getDefault(mView.getContext());
    }

    @Override
    public void setCameraOpenListener(CameraOpenDelegate cameraOpenListener) {
    ...
    @Override
    public void enableRecord(@NonNull MyRecorder recorder) {
    ...
    private void startCameraPreview() {
        try {
            ...
            postOpenCameraError(CameraOpenDelegate.CAMERA_DISABLED);
            e.printStackTrace();
            return;
        } catch (NoCameraException e) {
            postOpenCameraError(CameraOpenDelegate.NO_CAMERA);
            e.printStackTrace();
            return;
        }
        ...
        if (state != Cameras.State.PREVIEW) {
            try {
                ...
                e.printStackTrace();
                postOpenCameraError(CameraOpenDelegate.CAMERA_OPEN_FAILED);
            } catch (CameraNotSupportException e) {
                e.printStackTrace();
                postOpenCameraError(CameraOpenDelegate.CAMERA_NOT_SUPPORT);
            }
        }
    }
    ...
    private static class RenderRecord {
        ...
        void draw() {
            ...
            EGLContext mSavedEglContext = EGL14.eglGetCurrentContext();
            {
                ...
                if (mRecorder.isFirstSetup()) {
                    mRecorder.startSwapAsync();
